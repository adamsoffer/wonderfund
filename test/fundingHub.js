...
contract('FundingHub', function (accounts) {
  ...
  describe('FundingHub', function () {
    ...
    let imageUrl = 'http://virtualrealitytimes.com/wp-content/uploads/2017/04/3044381-ocie.jpg'
    ...
    before('should be possible to deploy FundingHub', function () {
      return FundingHub.new({ from: accounts[0] })
        .then(fundingHubInstance => {
          instance = fundingHubInstance
        })
    })

    before('should create a project with a deadline a week from now and keep timestamp', function () {
      return instance.createProject(projectName, description, imageUrl, beneficiary, deadline, fundingGoal, { from: beneficiary })
        .then(projectInstance => {
          project = Project.at('0x' + projectInstance.receipt.logs[0].data.substr(-40))
        })
    })

    before('should contribute 4 eth', function () {
      return instance.contribute(project.address, { from: contributor, value: contribution })
    })
      ...
      it('should refund', function () {
        ...
        return web3.evm.increaseTimePromise(0)
          ...
          .then((result) => {
            ...
            assert(web3.eth.getBalance(contributor).toNumber() > balanceBeforeRefund)
