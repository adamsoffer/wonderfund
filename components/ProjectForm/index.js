...
export default ({ project, handleSubmit }) => {
  ...
  } else if (moment(project.deadline).unix() < moment().unix() && !project.successfullyFunded) {
    body = (
      <Body>
        <Description>This project did not meet it's goal</Description>
        <Button onClick={() => { this.claimRefund() }}>Claim Refund</Button>
      </Body>
    )
  } else {
    body = (
      <Body>
        ...
        <Textfield label='Contribution (ETH):' type='number' name='contribution' placeholder='e.g 1' />
        <Button secondary fullWidth type='submit'>Fund</Button>
      </Body>
    )
  }
  return (
