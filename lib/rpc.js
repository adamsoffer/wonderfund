export default class RPC {
    ...
    return new Promise((resolve, reject) => {
      return this.web3.currentProvider.sendAsync(req, (err, result) => {
        if (err) {
          ...
          reject(
            new Error('RPC Error: ' + (result.error.message || result.error))
          )
        } else {
          resolve(result)
        }
  ...
  decreaseTime(time) {
    return this.sendAsync('evm_decreaseTime', [time])
  }
  ...
  snapshot() {
    return this.sendAsync('evm_snapshot').then(res => res.result)
  }
  ...
  async waitUntilNextBlockMultiple(blockMultiple, multiples = 1, seconds = 20) {
    ...
    await this.waitUntilBlock(
      this.nextBlockMultiple(currentBlock, blockMultiple) + additionalBlocks
    )
  }
